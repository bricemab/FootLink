...
export default function CoachesList(): ReactNode {
  ...
  const confirmRemove = (coach: Coach): void => {
    void act(async () => {
      ...
      setCoaches((current) => current?.filter((item) => item.clubMemberId !== coach.clubMemberId));
  ...
  return (
    ...
    >
      ...
      {coaches !== undefined && coaches.length === 0 ? <EmptyState text={t.coaches.empty} /> : null}
        ...
        return (
          <Appear key={coach.clubMemberId} index={index}>
            ...
            */}
              ...
              <Text
                {...TYPE.body}
                color={coach.teams.length > 0 ? '$brandChalk' : '$brandChalkDim'}
              >
...
function LinkAction({ label, onPress }: { label: string; onPress: () => void }): ReactNode {
  return (
    <Pressable onPress={onPress} accessibilityRole="button" hitSlop={8}>
      {({ pressed }) => (
        <Text {...TYPE.body} fontWeight="700" color="$brandPitchBright" opacity={pressed ? 0.6 : 1}>
          {label}
        </Text>
      )}
    </Pressable>
...
function InviteState({ coach }: { coach: Coach }): ReactNode {
  ...
  if (coach.inviteFailedAt !== null) {
    return (
      <Text {...TYPE.meta} fontWeight="700" color="#FFC14D">
