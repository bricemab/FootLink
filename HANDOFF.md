...
- 🔴 **JAMAIS `opacity: 0` dans le `from` d'une animation d'entrée qui enveloppe du CONTENU.** C'est la cause de tous les « écrans vides » signalés. Une animation Moti qui ne se joue pas laisse ses valeurs de départ en place : le bloc reste à `opacity: 0`, donc **invisible**, et l'écran paraît vide alors que tout est monté et fonctionnel. Le symptôme est **intermittent** (il dépend de l'état du thread JS au montage), donc très difficile à reproduire à la demande — signature reconnaissable : seuls les éléments **hors** MotiView restent visibles (dans le cas signalé, le stepper et « Retour », mais ni le titre, ni le sous-titre, ni le formulaire).
  - Corrigé en ne glissant plus que la position : `from={{ translateY: 18 }}`. Si l'animation manque, le contenu est juste en place — jamais absent.
  - Appliqué à `auth-form-shell` (titre + corps), `stepper` (`StepTransition`), `form-banner`, `text-field` (message d'erreur), `place-picker` (suggestions), `register/index` (cartes de rôle), `home`.
  - **Restent volontairement en `opacity: 0`** : `welcome.tsx` (Brice aime cet écran, le fondu en cascade est sa signature, et il n'a jamais blanchi) et `pitch-backdrop` (halos purement décoratifs). Risque résiduel assumé sur welcome.
