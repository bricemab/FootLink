...
69. 🔴 **Tester un module natif : `requireOptionalNativeModule`, JAMAIS un `require` dans un `try`.** Le paquet JS est dans `node_modules` des l'installation, donc le `require` REUSSIT toujours — ce qui manque tant que l'app n'est pas reconstruite, c'est le binaire natif, et l'erreur ne sort qu'au RENDU (« Cannot find native module ExpoImage »). Le premier repli d'`AppImage` ne protegeait donc rien, et Brice a pris l'ecran casse.
    - Le nom a passer est celui du module NATIF (`ExpoImage`), pas celui du paquet npm (`expo-image`).
    - ⚠️ Deux natures de repli selon l'endroit : dans un **gestionnaire d'evenement**, un `import()` paresseux suffit (`haptics.ts`, `expo-image-picker`) ; **rendu dans l'arbre**, il faut interroger le registre natif (`app-image.tsx`).
70. **Trois registres visuels, pas un seul cadre pour tout.** `Card` accepte `variant` : `hero` (halo vert porte, **UN SEUL par ecran**), `card` (un element, souvent tapable), `plain` (de l'information, aucun cadre). `SectionTitle` remplace le cadre quand il ne s'agissait que de regrouper. Brice : « des carres sous des carres, ca ne tape pas aux yeux ».
71. ⚠️ **Les animations d'entree sont possibles A CONDITION que leur etat de depart soit inoffensif.** `StepTransition` avait renonce a toute animation parce que, sur rendu logiciel, une entree ne se joue pas toujours et ses valeurs de depart PERSISTENT (`opacity: 0` → ecran vide, `translateX: 28` → contenu hors marge).
    - `Appear` part de `opacity 0.55` / `scale 0.98` / `translateY 6` : bloque, le contenu reste lisible et a sa place. C'est ce qui permet d'avoir enfin du mouvement sans reprendre le risque.
    - Les postes s'allument en cascade **en presentation uniquement** (`readOnly`) : pendant la saisie, une pastille qui grossit sous le doigt brouille le retour de l'appui.
    - ⚠️ Toujours pas de boucle : une surface animee en permanence recompose au CPU a chaque image sur l'emulateur (ANR deja constate).
72. **Geolocalisation a l'inscription joueur** (`GET /geo/here`). Elle n'avait jamais ete branchee : `expo-location` n'etait pas installe. Cote serveur il n'y avait presque rien a ecrire — `resolvePoint(lat, lng)` interrogeait deja swisstopo, il n'etait pas expose.
    - ⚠️ **`allowMyPosition` est reserve au JOUEUR**, desactive par defaut. Pour un club ce serait un contresens : on cherche l'emplacement de son TERRAIN, pas l'endroit ou il remplit le formulaire.
    - ⚠️ **Aucune vue satellite sur un point auto-localise.** Confirmer le terrain d'un club par une vue du ciel est utile ; la meme vue sur le DOMICILE d'un joueur serait tout autre chose. `aerialUrl` est devenu optionnel pour ca.
    - La position brute ne quitte l'appareil que pour cet appel ; c'est la COMMUNE qui est conservee, et le profil ne stocke que du ~1 km (AGENTS §6.5).
    - ⚠️ iOS reclamera une justification d'usage de la position (`NSLocationWhenInUseUsageDescription`) pour une soumission App Store. Sans effet sur un build de developpement.

66. 🔴 **La rotation du refresh token est SERIALISEE. Ne jamais retirer ce verrou.** Sans lui, deux requetes qui prennent un 401 en meme temps appellent toutes deux `refresh` avec le MEME jeton : la premiere le rotate, la seconde rejoue un jeton revoque — et depuis le correctif #10 de l'audit, rejouer un jeton rotate **revoque toute la famille**. L'utilisateur est alors deconnecte de force sans rien avoir fait.
    - Ce n'etait pas theorique : `home.tsx` demande le profil et le club en parallele (`Promise.all`), soit exactement le cas qui declenche la course. Le correctif de securite et cet ecran ont ete ecrits le meme jour, sans que le lien soit vu.
    - `rotateOnce` partage une promesse unique, liberee dans un `finally` — la garder apres un echec condamnerait la session jusqu'au redemarrage. Un appel arrive en file verifie d'abord si le jeton a deja change : si oui il repart de ce qui est en memoire au lieu de rejouer l'ancien.
67. **Une session ouverte interdit les ecrans d'authentification** (`useSignedInRedirect`). `router.replace` apres connexion ne suffisait pas : il change l'ecran, pas l'historique, et le bouton retour d'Android ramenait sur `/login` alors qu'on etait connecte.
    - ⚠️ La garde s'applique a `welcome`, `login`, `register/index` et `register/player` UNIQUEMENT. `register/club` et `register/coach` s'utilisent AVEC une session ouverte (l'identite vient d'abord, AGENTS §4bis) et `index.tsx` y renvoie lui-meme : les guarder creerait une boucle.
    - ⚠️ La condition reprend **exactement** celle de la premiere branche d'`index.tsx` (`phase === 'signedIn'` ET `user` present). Un critere plus large et les deux se renverraient la balle.
    - Verifie sur l'AVD : le retour sort de l'application au lieu de revenir sur la connexion, et la session est restauree telle quelle a la reouverture.
68. **Les ecrans se relisent au RETOUR, pas seulement au montage** (`useFocusEffect` et non `useEffect`). Un ecran qu'on quitte reste monte : en revenant apres avoir cree une annonce, on retrouvait la liste d'avant — « 0 annonce » alors qu'on venait d'en creer une. Applique aux huit ecrans qui lisent des donnees.
    - Le stockage des jetons, lui, n'a pas bouge : SecureStore (Keychain/Keystore) survit deja aux mises a jour de l'application. Le probleme n'etait pas la.

62. **Le terrain est la representation du joueur, pas un champ de saisie** (27 juillet 2026). `home.tsx` etait un panneau de diagnostic — email, `PLAYER`, `ACTIVE`, les codes bruts des enums. Il porte desormais la fiche : nom, photo, statut de recherche, et le terrain avec les postes allumes.
    - ⚠️ `PitchPositions` accepte `readOnly`. Sans lui, les pastilles restaient pressables sur un ecran de presentation : elles repondaient au toucher, declenchaient un retour haptique et ne changeaient rien. Un element qui a l'air actionnable sans l'etre fait douter du reste de l'ecran.
    - Le poste principal n'est PAS repete dans la carte d'informations : `PitchPositions` l'annonce deja sous le dessin.
    - ⚠️ `PlayerProfileResponse` cote mobile etait **trop etroit** : le serveur renvoyait deja bio, pied fort, taille, club et drapeaux de visibilite. L'ecran se rabattait sur l'email faute de savoir qu'il avait mieux. Verifier le service avant de conclure qu'une donnee manque.
    - `strongFootLabel` ajoute dans `packages/shared` : l'ecran s'appretait a afficher `DROIT` en majuscules. **Les codes d'enum sont pour la base, jamais pour l'ecran** — meme regle que `posteLabel` et `categoryLabel`.
63. ⚠️ **Deux modules natifs ajoutes derriere un repli** : `expo-image` et `expo-haptics`. Ils imposent une reconstruction, et l'app doit fonctionner AVANT comme APRES — sinon tout ecran affichant une image tombe tant que le build n'a pas suivi.
    - **Les deux replis ne sont pas de la meme nature.** L'haptique est appelee dans un gestionnaire d'evenement : un `import()` paresseux suffit. Une image est rendue **dans l'arbre** : `AppImage` tente donc un `require` une fois et retombe sur l'`Image` de React Native.
    - Retour haptique sur les gestes qui ENGAGENT seulement (publier, clore, allumer un poste) et sur les echecs, avec un motif distinct. ⚠️ **Jamais sur chaque appui** : un bourdonnement permanent finit coupe dans les reglages systeme, et on perd alors aussi les notifications qui comptent.
64. **Squelettes de chargement** (`ui/skeleton.tsx`) au lieu des roues dans les listes, et `EmptyState` illustre d'un demi-terrain — **le meme trace que `PitchPositions`**, pour que l'absence et la presence parlent la meme langue.
    - ⚠️ L'animation des squelettes boucle en **opacite seulement**. Animer la geometrie d'une pile de blocs sur un rendu logiciel (l'emulateur) fait grimper le RenderThread, ce qui a deja valu un « FootLink isn't responding ».
65. **Compte joueur de demonstration** : `joueur-demo@e2e.footlink.test` / `FootLink2026`, cree directement en base (email deja verifie). Passer par l'inscription reelle exigerait un code par email, et Gmail met des dizaines de secondes. ⚠️ Il n'existait AUCUN compte joueur avant : toute verification a l'ecran cote joueur en demande un.

61. **Plafond d'invitations : 3 par entraineur et par 30 minutes** (`COACH_INVITE_MAX_PER_WINDOW`). Verifie sur l'API : 3 passes (107, 83, 65 ms), la 4e refusee en 6 ms avec `COACH_INVITE_RATE_LIMITED`.
    - ⚠️ **Limite PAR ENTRAINEUR, distincte du `ThrottlerGuard` par IP.** Les deux sont necessaires : le throttle protege le serveur, celui-ci protege le destinataire — un club avec vingt entraineurs passerait sous le throttle tout en inondant une seule boite.
    - Compte **tous** les emails d'invitation de la fenetre, creation du compte comprise : l'objectif est de ne pas inonder une adresse, pas de compter des clics.
    - ⚠️ La garde est posee **AVANT** de creer le jeton. L'inverse invaliderait le code precedent sans en delivrer de nouveau, et laisserait l'entraineur avec un code mort.
    - Cote app, la confirmation « nouveau code envoye » s'efface apres 3 s et le bouton revient : un renvoi peut legitimement etre repete. Le minuteur est dans un `ref`, annule avant d'en armer un autre et au demontage.

59. ⚠️ **L'envoi d'email ne bloque PLUS la reponse HTTP** (27 juillet 2026). Creer un entraineur repondait en 20 a 85 s, parce que le service attendait le `sendMail`. Mesure apres decouplage : **90 ms** au lieu de 30-85 s.
    - **L'echec devient muet, il fallait donc le rattraper.** `ClubMember.inviteSentAt` / `inviteFailedAt` sont horodates par la tache de fond et remontes dans la fiche de l'entraineur (« en cours d'envoi », « envoyee », « l'envoi a echoue »). Sans ca on recreait exactement le defaut reproche au repli SMTP par l'audit (#2) : un club croirait son entraineur invite.
    - ⚠️ **Des DATES, pas un message d'erreur** : le detail SMTP peut contenir l'adresse du relais et des messages de diagnostic, il reste dans les logs serveur.
    - ⚠️ **Le jeton reste cree de facon SYNCHRONE** au renvoi : c'est lui qui invalide le precedent, et le club doit pouvoir compter dessus des que l'API a repondu. Seul l'envoi part en arriere-plan.
60. ⚠️ **Gmail SMTP est lent et irregulier depuis un poste de dev** : 30 a 85 s pour un seul message, mesure identique en 587+STARTTLS, en 465, et avec un pool (le 2e message d'une connexion ouverte etait meme plus lent). **Ce n'est pas le code** : un script autonome, qui ne touche a aucune ligne de FootLink, mesure la meme chose. Le meme envoi est retombe a ~10 s une heure plus tard, ce qui ressemble a une limitation de debit cote Gmail apres une rafale de messages de test.
    - Conclusion pour la production : **Gmail SMTP n'est pas un transport transactionnel**. Plafond de ~500 envois par jour, latence non garantie. Un service HTTP (Resend, Brevo, Postmark) repond en 200-500 ms. Le decouplage ci-dessus rend le probleme invisible a l'utilisateur, il ne le supprime pas — decision a prendre avant la mise en production.

55. 🔐 **Audit de securite du 26 juillet 2026 — correctifs appliques.** Le rapport complet est dans `SECURITY_AUDIT.md` (dans la copie `FootLink/`, non versionnee). Aucune vulnerabilite critique. Ce qui a ete corrige, et ce qui ne l'a pas ete :
    - **#1 Injection HTML dans les emails.** `clubName` (modifiable par le CLUB_ADMIN) et `firstName` etaient interpoles sans echappement dans des emails brandes FootLink → phishing possible sous notre identite. `escapeHtml` dans `mail.service.ts`. ⚠️ **Les SUJETS ne sont pas echappes, exprès** : un sujet est du texte brut, `&amp;` s'y afficherait litteralement.
    - **#2 Fail-open SMTP.** Sans SMTP, l'API tombait en mode simule et journalisait **les jetons en clair**. Desormais `validateEnv` **refuse de demarrer** en production sans SMTP, et `MailService` re-verifie au boot. Le mode simule reste en dev/test : les scripts `tools/e2e` relisent le format `[email simulé] … | token=…` dans les logs — **ne pas changer ce format**.
    - **#3 CORS.** `credentials: false` (l'auth est par header Bearer, aucun cookie) et refus de demarrer si `CORS_ORIGINS` contient `*` en production. Le `*` reste tolere en dev.
    - **#10 Rejeu de refresh token.** Rejouer un jeton deja rotate revoque maintenant **toute la famille** de l'utilisateur. Avant, le voleur et la victime gardaient chacun une session valide, sans alerte.
    - **#11 TOCTOU.** Incrementations et consommations de jetons rendues atomiques (`updateMany` avec la condition dans le `WHERE`, puis test de `count`).
    - **#12** en-tetes de securite poses a la main (pas de helmet) : `nosniff`, `DENY`, `no-referrer`, CSP `default-src 'none'` — assouplie pour `/l/` uniquement, seule page HTML servie.
    - **#13 Verrou de compte au login** (`User.failedLoginAttempts`, `loginLockedUntil`) + **mot de passe a 10 caracteres minimum**. ⚠️ `LoginDto` **n'a pas** de `MinLength` : les comptes crees avant le durcissement continuent de se connecter. Ne pas « harmoniser » ce DTO, ce serait verrouiller dehors les comptes existants.
    - **#16** `RECORD_AUDIO` et `SYSTEM_ALERT_WINDOW` bloquees via `android.blockedPermissions` dans `app.json` (et non dans le manifeste, qui est regenere).
    - **#4 `email_verified` de Google : RISQUE ACCEPTE** par Brice, sans changement de code. A reevaluer si un autre fournisseur d'identite est ajoute.
    - **#6 / #7 : premisse fausse.** L'audit a vu `apps/mobile/android/` et `debug.keystore` parce que la copie a ete faite **sans respecter `.gitignore`**. Ce dossier est ignore : c'est un artefact de `expo prebuild`, pas du code versionne. Le risque reel restant est local — ne jamais lancer `gradlew assembleRelease` depuis ce dossier, la signature serait celle du keystore de debug.
    - **Non traites** : #5 (migration vers App Links / Universal Links = chantier a part ; l'auto-soumission du lien profond a en revanche ete **retiree**, le jeton ne fait plus que pre-remplir le champ), #14, #15, #17, #18.
56. ⚠️ **`NOW()` en SQL brut n'est PAS l'heure de Prisma.** MySQL renvoie l'heure **locale** du serveur (+02:00 ici) et Prisma relit le `DATETIME` comme de l'**UTC** : une date posee a `NOW()` se relit **deux heures dans le futur**. Un « verrou expire d'une seconde » ecrit en SQL apparaissait donc toujours actif, et le controle accusait le code applicatif. Les scripts e2e utilisent des ecarts d'au moins un jour. Le code applicatif n'est pas concerne — il n'ecrit ces dates que via Prisma, donc en UTC de bout en bout.
57. **`tools/e2e/security.ts`** couvre les correctifs de l'audit (18 controles). ⚠️ **A lancer SEUL** : `POST /auth/login` a son propre seau de 10 requetes/minute/IP et cette suite en consomme 7.
    - Le verrou de compte est verifie par son **comportement**, jamais en lisant le compteur — `prisma db execute` ne retourne pas de lignes. On force l'etat par des `UPDATE` **conditionnels** et c'est la reponse de l'API qui revele si la condition portait.
    - ⚠️ **L'echappement HTML des emails n'est pas couvert** : le transport simule ne journalise que sujet, destinataire et jeton, jamais le corps HTML. Ce correctif ne repose que sur la relecture du code.
    - Constat au passage : avec un throttle a 10/min par IP, **le verrou a 10 echecs est inatteignable depuis une seule adresse**. Il ne sert que contre du credential stuffing distribue — ce qui est bien sa raison d'etre, mais explique qu'on ne puisse pas le declencher « normalement ».
58. **Le decor traverse l'en-tete et la barre d'onglets ; le contenu, non.** Demande de Brice le 26 juillet. Le fond de marque est peint **une seule fois** par `BackdropRoot` a la racine de l'espace club, donc derriere l'en-tete et la barre, qui n'ont plus de fond opaque. Les halos ne s'arretent plus net a leur bord.
    - Un contexte `InsideChrome` porte le fait « un habillage m'encadre et possede deja les zones de securite ». **Trois consequences d'un seul fait** : l'ecran ne repeint pas de fond, n'ajoute pas `insets.top` (l'en-tete l'a consomme), n'ajoute pas `insets.bottom` (la barre le reserve). Ce dernier point creusait un trou de **deux** zones de securite entre le dernier bouton et les icones.
    - ⚠️ **Trois endroits doivent etre transparents, et on ne les trouve qu'en regardant l'ecran** : `sceneStyle` du navigateur d'onglets, **et** le `contentStyle` de **chaque** pile imbriquee (`teams`, `coaches`, `listings`). Le `sceneStyle` ne couvre que le conteneur, pas les cartes des piles qui vivent dedans — symptome : deux onglets avec le decor, un troisieme au fond plat.
    - ⚠️ Le voile du repli `expo-blur` est passe de 0.45 a **0.18** et le fond opaque de `GlassSurface` a ete **supprime**. C'etait justifie tant que du contenu clair defilait derriere la barre ; ca ne l'est plus. **Si la barre redevenait flottante, il faut le remettre.**
